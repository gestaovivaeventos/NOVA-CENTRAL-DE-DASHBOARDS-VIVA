/**
 * Gráfico de Fundos Ativos por Mês com comparativo de anos
 * Estilo igual ao MultiYearBarChart do módulo vendas
 */
...
interface FundosMensalChartProps {
  data: {
    year: number;
    monthlyData: number[];
  }[];
...
export default function FundosMensalChart({
  data,
  activeYears,
  onYearToggle,
  suffix = '',
  showSuffixOnScale = false,
}: FundosMensalChartProps) {
    ...
    scales: {
      ...
      y: {
        ...
        ticks: {
          color: '#F8F9FA',
          font: { size: 14, family: 'Poppins, Arial, sans-serif' },
          callback: (value: any) => formatNumber(Number(value)) + (showSuffixOnScale ? suffix : ''),
        },
        grid: { color: 'rgba(255,255,255,0.04)' },
      },
    },
  }), [suffix, showSuffixOnScale]);
