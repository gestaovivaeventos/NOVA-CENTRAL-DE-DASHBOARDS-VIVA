...
import { TeamPerformance, KpiData, NovoOkrData } from '../types';
...
export const TeamPerformanceTable: React.FC<TeamPerformanceTableProps> = ({ 
  ...
  // Tipo para OKRs agrupados por objetivo
  ...
  const getOkrsAgrupados = (teamOkrs: NovoOkrData[]): OkrAgrupado[] => {
    ...
    // Ordenar por ID_OKR e depois ID_KR
  ...
  // Agrupar OKRs por objetivo estratégico
  const getOkrsPorObjetivo = (okrsAgrupados: OkrAgrupado[]): OkrsPorObjetivo[] => {
    const grupos: Record<string, { idOkr: number; indicadores: OkrAgrupado[] }> = {};
    
    okrsAgrupados.forEach(okr => {
      const obj = okr.objetivo || 'Sem Objetivo';
      if (!grupos[obj]) {
        grupos[obj] = { idOkr: okr.idOkr, indicadores: [] };
      }
      grupos[obj].indicadores.push(okr);
    });
    
    // Ordenar por ID_OKR e indicadores por ID_KR
                            ...
                            {selectionMode === 'kpi' ? (
                              ...
                              <>
                                {(() => {
                                  ...
                                  const okrsPorObjetivo = getOkrsPorObjetivo(okrsAgrupados);
                                  return (
                                    <>
                                      <h4 className="text-orange-400 font-semibold mb-3">
                                        OKRs - {team.time} ({okrsAgrupados.length} indicadores)
                                      </h4>
                                      {okrsPorObjetivo.length > 0 ? (
                                        <div className="overflow-x-auto">
                                          <table className="w-full text-sm">
                                            ...
                                            <tbody>
                                              {okrsPorObjetivo.map((grupo, grupoIndex) => (
                                                <React.Fragment key={grupoIndex}>
                                                  {/* Título do Objetivo Estratégico - discreto */}
                                                  <tr className="bg-slate-800/20">
                                                    <td 
                                                      colSpan={4} 
                                                      className="py-2 px-3 text-slate-500 text-xs uppercase tracking-wide border-b border-slate-700/50"
                                                    >
                                                  ...
                                                  {grupo.indicadores.map((okr, okrIndex) => {
                                                    return (
                                                      ...
                                                      >
                                                        <td className="py-2 px-3 pl-6 text-slate-300">{okr.indicadorNome}</td>
