/**
 * Modal para editar um grupo (nome, ícone, ordem, ativo)
 * Abre ao clicar no lápis do header de grupo na tabela
 */
...
interface EditGrupoModalProps {
  ...
  gruposDaPlanilha: GrupoInfo[];
  onSave: (grupoOriginal: string, updates: { nome?: string; icone?: string; ordem?: number; ativo?: boolean }) => Promise<boolean>;
}
...
export default function EditGrupoModal({
  ...
  useEffect(() => {
    if (isOpen && grupoNome) {
      const info = gruposDaPlanilha.find(
  ...
  }, [isOpen, grupoNome, gruposDaPlanilha]);

  const handleSave = async () => {
    if (!grupoNome || !nome.trim()) return;

    // Verificar duplicata
    if (nome.trim().toLowerCase() !== grupoNome.toLowerCase()) {
      const dup = gruposDaPlanilha.find(
        (g) =>
          g.nome.toLowerCase() === nome.trim().toLowerCase() &&
          g.nome.toLowerCase() !== grupoNome.toLowerCase()
      );
      if (dup) {
        setFeedback({ type: 'error', msg: `Grupo "${nome.trim()}" já existe` });
        return;
      }
    }
    ...
    const ok = await onSave(grupoNome, {
      nome: nome.trim(),
      icone,
      ordem,
      ativo,
    ...
    if (ok) {
      setFeedback({ type: 'success', msg: 'Grupo atualizado!' });
  ...
  if (!isOpen || !grupoNome) return null;
      ...
      >
        ...
        <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: 20, overflowY: 'auto' }}>
          ...
          <div style={{ display: 'flex', gap: 16 }}>
            <div style={{ flex: 1 }}>
              <label style={labelStyle}>Ordem na Sidebar</label>
              <input
                type="number"
                min={1}
                max={99}
          ...
          {!ativo && (
            ...
            >
              Grupo inativo não será exibido na sidebar da página inicial.
            </p>
          )}
        </div>
