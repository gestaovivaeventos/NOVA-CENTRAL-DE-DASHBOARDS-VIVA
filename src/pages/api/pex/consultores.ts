...
const CACHE_KEY = 'pex:consultores';
...
export default async function handler(
  ...
  try {
    ...
    if (req.method === 'GET') {
      try {
        ...
        if (forceRefresh) {
          ...
          console.log('[Consultores API] Cache invalidado por forceRefresh');
        }
    ...
    if (req.method === 'POST') {
      ...
      invalidateCache(CACHE_KEY);
      console.log(`[Consultores API] Cache invalidado após atualização de ${unidade}`);
