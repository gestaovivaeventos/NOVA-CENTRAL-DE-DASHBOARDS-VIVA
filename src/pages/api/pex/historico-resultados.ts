/**
 * API Route - Histórico Resultados
 * Retorna dados oficiais de resultados por franquia (aba HISTORICO RESULTADOS)
 * Colunas relevantes: A(data), B(Franquia), F(VVR), T(VVR Carteira),
 * AG(Endividamento), AJ(NPS), AN(E-NPS), AO(Margem), AQ(Reclame Aqui),
 * AT(colab_1ano), AU(Estrutura), AY(Churn), AZ(Conformidades), BA(tempo_medio_carteira)
 */
...
export default async function handler(
  ...
  try {
    ...
    // Buscar até coluna BA (53 colunas)
    ...
    console.error('Erro ao buscar dados do histórico de resultados:', error);
    res.status(500).json({ error: 'Erro ao buscar dados do histórico de resultados', message: String(error) });
  }
}
