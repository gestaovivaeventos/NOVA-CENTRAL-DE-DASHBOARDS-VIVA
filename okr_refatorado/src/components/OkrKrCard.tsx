...
export const OkrKrCard: React.FC<OkrKrCardProps> = ({
  ...
  const handleSaveData = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/update-okr', {
        method: 'POST',
        headers: {
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          ...
          displayColors: false,
          callbacks: {
            label: (context: { dataset: { label?: string }; dataIndex: number; raw: unknown }) => {
        ...
        {!showDataEntry && (
          <div className="chart-wrapper">
            ...
            <div className="main-chart-area">
              <Bar ref={chartRef} data={mainChartConfig.data} options={mainChartConfig.options as never} />
            </div>
          </div>
        )}
      </div>
