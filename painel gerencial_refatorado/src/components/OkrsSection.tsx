...
export const OkrsSection: React.FC<OkrsSectionProps> = ({ okrs, competencia }) => {
  ...
  // Se não houver resultado no mês da competência, busca o último mês apurado disponível
  const findLastResultForKR = (indicator: string): OkrData | null => {
    // Filtrar todos os OKRs deste indicador
    ...
    // Se não encontrou resultado exato, buscar o último resultado disponível
    // que seja anterior ou igual ao mês da competência (mesmo ano ou anos anteriores)
  ...
  // Filtrar OKRs pela competência OU buscar o último resultado do quarter
  // Agrupar por indicador para evitar duplicatas
  ...
  // Encontrar a meta mais recente (última data) para cada indicador
  ...
  // Calcular atingimento do ano (realizado / meta anual)
  ...
  const objectiveMetrics = objectives.map((objective, index) => {
    ...
    // Calcular média do atingimento do ano
    ...
    <div className="mb-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        {objectiveMetrics.slice(0, 2).map((obj) => {
          ...
          // Definir cores do gradiente baseado no status
          ...
          // Calcular o ângulo do arco (0-360 graus)
